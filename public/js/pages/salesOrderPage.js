  ...
  window.MKApp.registerPage('sales-orders', {
    ...
    template: function () {
      return '<div class="grid-2">'
        ...
        + '<div class="card"><h3>Danh sách đơn</h3><div class="grid"><div><label>Tìm kiếm</label><input id="soQ" placeholder="Mã đơn / khách hàng"></div><div><label>Ngày giao</label><input id="soDeliveryDate" type="date"></div><div><label>Trạng thái</label><select id="soStatus"><option value="all">Tất cả</option><option value="pending">pending</option><option value="assigned">assigned</option><option value="delivered">delivered</option><option value="cancelled">cancelled</option></select></div></div><div class="actions"><button id="loadSalesOrdersBtn" class="primary">Tải đơn</button><button id="clearSoDateBtn">Bỏ lọc ngày</button></div><div id="salesOrderPerf" class="perf"></div><div class="table-wrap"><table><thead><tr><th>Mã</th><th>Khách</th><th>Ngày giao</th><th>Tiền</th><th>Trạng thái</th><th>Kế toán</th></tr></thead><tbody id="salesOrderRows"></tbody></table></div></div></div>';
    },
