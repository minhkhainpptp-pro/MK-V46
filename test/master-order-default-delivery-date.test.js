...
test('master order frontend initializes creation and delivery dates independently', () => {
  const js = fs.readFileSync(path.join(root, 'public/js/app/06-master-delivery.js'), 'utf8');
...
test('master order service stores immutable masterOrderDate and server-side default delivery date', () => {
  const service = fs.readFileSync(path.join(root, 'src/services/master-order/masterOrderLegacy.service.js'), 'utf8');
